...
def runTraining(args):
    ...
    if args.model == 'swin_daf':
        net = SwinDAF(
            num_classes=5,
            encoder_name=args.swin_encoder,
            pretrained=not args.no_pretrain,
        )
        print(" Encoder: {}  pretrained={}".format(args.swin_encoder, not args.no_pretrain))
    else:
        net = DAF_stack()
    ...
    for i in range(epoch):
        ...
        if currentDice > BestDice:
            ...
            print("###    In 3D -----> MEAN: {:.4f}, Dice(1): {:.4f} Dice(2): {:.4f} Dice(3): {:.4f} Dice(4): {:.4f}   ###".format(np.mean(mean_DSC3D),mean_DSC3D[0], mean_DSC3D[1], mean_DSC3D[2], mean_DSC3D[3]))
...
if __name__ == '__main__':
    ...
    # Model selection
    parser.add_argument('--model', default='daf', choices=['daf', 'swin_daf'],
                        help='daf = original ResNeXt-101 backbone | swin_daf = Swin Transformer backbone')
    parser.add_argument('--swin_encoder', default='swin_tiny_patch4_window7_224',
