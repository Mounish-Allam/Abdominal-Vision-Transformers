...
def evaluate3D(modelName):
    ...
    # If GT NIfTI folder is missing, skip 3D evaluation and return zeros
...
def reconstruct3D(modelName,epoch, isBest=False):
    ...
    for s_i in range(len(subjNames)):
         ...
         # Detect actual image size from the first saved slice
         _probe = Image.open(path_Subj + '/1.png').convert('LA')
         xSize, ySize = _probe.size[1], _probe.size[0]
