...
class SwinDAF(nn.Module):
    """
    Dual Attention Fusion with a Swin Transformer encoder.

    Architecture:
      - Encoder : Swin-T/S/B  (replaces ResNeXt-101)
      - Decoder : original stacked PAM + CAM + semantic-guided attention

    The decoder is kept identical to DAF_stack so that the two models can be
    compared directly.  Only the channel widths of the four down-projection
    layers change to match Swin's output dimensions.
    """
    ...
    def __init__(
        ...
        self.encoder = SwinEncoder(encoder_name, pretrained=pretrained)
        ch = self.encoder.channels  # e.g. [96, 192, 384, 768] for swin_tiny

        # Project each Swin stage down to 64 channels
        self.down4 = nn.Sequential(nn.Conv2d(ch[3], 64, 1), nn.BatchNorm2d(64), nn.PReLU())
        self.down3 = nn.Sequential(nn.Conv2d(ch[2], 64, 1), nn.BatchNorm2d(64), nn.PReLU())
        self.down2 = nn.Sequential(nn.Conv2d(ch[1], 64, 1), nn.BatchNorm2d(64), nn.PReLU())
        self.down1 = nn.Sequential(nn.Conv2d(ch[0], 64, 1), nn.BatchNorm2d(64), nn.PReLU())

        out_channels = 64

        # ── Attention branch 1 (first stacked attention) ─────────────────────
        ...
        # ── Attention branch 2 (second stacked attention) ────────────────────
        ...
        # ── Prediction heads ─────────────────────────────────────────────────
        self.predict4   = nn.Conv2d(64, num_classes, 1)
        self.predict3   = nn.Conv2d(64, num_classes, 1)
        self.predict2   = nn.Conv2d(64, num_classes, 1)
        self.predict1   = nn.Conv2d(64, num_classes, 1)
        self.predict4_2 = nn.Conv2d(64, num_classes, 1)
        self.predict3_2 = nn.Conv2d(64, num_classes, 1)
        self.predict2_2 = nn.Conv2d(64, num_classes, 1)
        self.predict1_2 = nn.Conv2d(64, num_classes, 1)

    def forward(self, x: torch.Tensor):
        # ── Encoder ──────────────────────────────────────────────────────────
        if x.shape[1] == 1:          # grayscale → repeat to 3 channels
            x = x.repeat(1, 3, 1, 1)
        s1, s2, s3, s4 = self.encoder(x)   # strides: 4, 8, 16, 32

        target_size = s1.size()[2:]
        down4 = F.interpolate(self.down4(s4), size=target_size, mode='bilinear', align_corners=True)
        down3 = F.interpolate(self.down3(s3), size=target_size, mode='bilinear', align_corners=True)
        down2 = F.interpolate(self.down2(s2), size=target_size, mode='bilinear', align_corners=True)
        down1 = self.down1(s1)
        ...
        out_size = x.size()[2:]
        predict1   = F.interpolate(predict1,   size=out_size, mode='bilinear', align_corners=True)
        predict2   = F.interpolate(predict2,   size=out_size, mode='bilinear', align_corners=True)
        predict3   = F.interpolate(predict3,   size=out_size, mode='bilinear', align_corners=True)
        predict4   = F.interpolate(predict4,   size=out_size, mode='bilinear', align_corners=True)
        predict1_2 = F.interpolate(predict1_2, size=out_size, mode='bilinear', align_corners=True)
        predict2_2 = F.interpolate(predict2_2, size=out_size, mode='bilinear', align_corners=True)
        predict3_2 = F.interpolate(predict3_2, size=out_size, mode='bilinear', align_corners=True)
        predict4_2 = F.interpolate(predict4_2, size=out_size, mode='bilinear', align_corners=True)

        if self.training:
            return (
                semVector_1_1, semVector_2_1,
                semVector_1_2, semVector_2_2,
                semVector_1_3, semVector_2_3,
                semVector_1_4, semVector_2_4,
                ...
                semanticModule_1_4, semanticModule_1_3,
                semanticModule_1_2, semanticModule_1_1,
                semanticModule_2_4, semanticModule_2_3,
                semanticModule_2_2, semanticModule_2_1,
